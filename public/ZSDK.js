...
var ZSDKMessageManager = (function (ZSDKMessageManager) {
  ...
  var AllowedOrigins = new RegExp(
    "^https?://[a-zA-Z0-9-_]*.(sandbox.crm-oem.com|csez.zohocorpin.com|zoho.com|zoho.eu|zohoplatform.com|zohosandbox.com)(:[0-9]{0,4})?$"
  );
  ...
  function isAuthorizedMessage(MEvent) {
    ...
    if (
      SDKContext.isAppRegistered() &&
      AuthParentWindow === incomingSource &&
      AuthParentOrigin === incomingOrigin
    ) {
      return true;
    }
  ...
  function RegisterApp() {
    ...
    window.parent.postMessage(
      registerSDKClient,
      SDKContext.QueryParams.serviceOrigin
    );
  }
...
var ZSDKEventManager = (function (ZSDKEventManager) {
  ...
  function NotifyEventListeners(AppContext, eventName, eventData) {
    ...
    var isInternalEvent =
      Array.isArray(internalEventCheck) && internalEventCheck.length > 0;
...
function ZSDK() {
  ...
  function ExecuteLoadHandler() {
    if (typeof AppCode !== "function") {
      Logger.Error("No OnLoad Handler provided to execute.");
      return;
    }
    if (isOnLoadTriggered) {
      Logger.Error("OnLoad event already triggered.");
      return;
    }
  ...
  function Translate(key, options) {
    ...
    if (options) {
      ...
      for (key in keysArr) {
        valStr = _replaceString(
          valStr,
          "${" + keysArr[key] + "}",
          translateOptions[keysArr[key]]
        );
      }
    }
  ...
  function InitI18n() {
    ...
    for (var i in all) {
      if (all.hasOwnProperty(i)) {
        var valStr = _getKeyByString(
          localeResource,
          all[i].getAttribute("data-i18n")
        );
        if (!valStr) {
          return false;
        }
        ...
        if (all[i].hasAttribute("data-options")) {
          var options = JSON.parse(
            JSON.stringify(
              eval("(" + all[i].getAttribute("data-options") + ")")
            )
          );
